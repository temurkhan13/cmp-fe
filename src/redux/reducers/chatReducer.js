    ...
    case actionTypes.ADD_SHARED_USER:
      return state.map(chat => {
        if (chat.chatId === action.payload.chatId) {
          return {
            ...chat,
            sharedUsers: [...chat.sharedUsers, { userId: action.payload.userId, role: action.payload.role }]
          };
        }
        return chat;
      });
    ...
    case actionTypes.ADD_COMMENTING_USER:
      return state.map(chat => {
        if (chat.chatId === action.payload.chatId) {
          return {
            ...chat,
            commentingUsers: [...chat.commentingUsers, { userId: action.payload.userId, userName: action.payload.userName }]
          };
        }
        return chat;
      });
